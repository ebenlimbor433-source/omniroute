...
const db = (pattern: RegExp) => evidence("tests/unit/exclusive/exclusive-connection-leases.test.ts", pattern);
const auth = (pattern: RegExp) => evidence("tests/unit/sse/sse-auth-exclusive-leases.test.ts", pattern);
const chat = (pattern: RegExp) =>
  evidence("tests/unit/chat/chat-managed-lease-routing.test.ts", pattern);
const route = (pattern: RegExp) => evidence("tests/unit/session/session-leases-route.test.ts", pattern);
const context = (pattern: RegExp) => evidence("tests/unit/context/lease-context.test.ts", pattern);
const isolation = (pattern: RegExp) =>
  evidence("tests/unit/exclusive/exclusive-lease-auxiliary-isolation.test.ts", pattern);
const inventory = (pattern: RegExp) =>
  evidence("tests/unit/session/hard-session-lease-bypass-inventory.test.ts", pattern);
const managedSet = (pattern: RegExp) =>
  evidence("tests/unit/exclusive/exclusive-lease-managed-set.test.ts", pattern);
const connectionIsolation = (pattern: RegExp) =>
  evidence("tests/unit/exclusive/exclusive-lease-connection-test-isolation.test.ts", pattern);
const ws = (pattern: RegExp) =>
  evidence("tests/unit/codex/codex-ws-policy-enforcement-6564.test.ts", pattern);
const internalKey = (pattern: RegExp) =>
  evidence("tests/unit/api/pick-internal-api-key-6372.test.ts", pattern);
const requestLogger = (pattern: RegExp) =>
  evidence("tests/unit/request/request-logger-endpoints.test.ts", pattern);
const executorHeaders = (pattern: RegExp) =>
  evidence("tests/unit/chatcore/chatcore-executor-client-headers.test.ts", pattern);
...
const GATES = new Map<number, GateEvidence[]>([
  ...
  [68, [evidence("tests/unit/sse/sse-auth.test.ts", /session .*affinity/i)]],
  ...
  [91, [evidence("tests/unit/chat/chat-context-relay.test.ts", /context-relay/i)]],
  ...
  [
    ...
    [
      evidence(
        "tests/unit/session/hard-session-lease-zero-model-gates.test.ts",
