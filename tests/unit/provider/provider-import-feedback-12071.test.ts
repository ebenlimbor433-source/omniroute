...
const feedback =
  await import("../../../src/app/(dashboard)/dashboard/providers/components/providerImportFeedback.ts");
const { parseProviderImportFile } =
  await import("../../../src/app/(dashboard)/dashboard/providers/components/parseProviderImportFile.ts");
    ...
    errors: [
      {
        index: 1,
        name: "srv-107",
        provider: "openai-compatible-chat-001",
...
test("#12071 formatImportErrorLine prefers name, then provider, then 1-based row", () => {
  assert.equal(
    feedback.formatImportErrorLine({
      name: "Grade-S-Node",
      message: "Unknown or unsupported provider",
    }),
...
test("#12071 applyImportHttpOutcome on ok with success>0 requests refresh", () => {
  const outcome = feedback.applyImportHttpOutcome(
    { ok: true, status: 200 },
    {
      ...
      failed: 1,
      total: 3,
      errors: [{ name: "bad", message: "Unknown or unsupported provider" }],
    }
  );
